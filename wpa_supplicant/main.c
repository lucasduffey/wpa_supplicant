...
int main(int argc, char *argv[])
{
	...
	for (unsigned int idx = 0; exitcode == 0 && idx < iface_count; idx++) {
		struct wpa_supplicant *wpa_s;

		if ((ifaces[idx].confname == NULL &&
		     ifaces[idx].ctrl_interface == NULL) ||
		    ifaces[idx].ifname == NULL) {
		...
		wpa_s = wpa_supplicant_add_iface(global, &ifaces[idx], NULL);
		if (wpa_s == NULL) {
			exitcode = -1;
			break;
		}
